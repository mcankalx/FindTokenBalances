...
{
  // Reading the wallet address and token address passed in the request URL
  ...
  const isItDoneYet = new Promise( ( resolve, reject ) =>
    {
      web3.alchemy.getTokenBalances( address, tokenAddress ).then( balances =>
      {
        ...
        balances = JSON.parse( JSON.stringify( balances, 0, 2 ) );
...
var server = app.listen( process.env.PORT || global.gConfig.node_port, function ()
{
  ...
  console.log( "Fetch getTokenBalance Details app listening at %s", port )
